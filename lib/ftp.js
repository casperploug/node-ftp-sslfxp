...
module.exports = function ftp(options) {
  ...
  this.ondir = function() {};
  this.read_callback = null; // buffering response
  ...
  this._read_encrypted = function(data) {
    ...
    if (root.buffering) {
      ...
      if (!root.res_buffer.match(/([0-9]{3}[ ][^\n]+$|[\n\r][0-9]{3}[ ][^\n]+\n)/)) {
        return;
      } else { root.buffering = false; }
    ...
    for (var i = 0, j = ftp_data.length; i < j; i++) {
      if (root.read_callback) {
        root.read_callback(ftp_data[i].msg);
        continue;
      }
      switch (ftp_data[i].code) {
        case 1: // dirlist
      ...
      if (root.read_callback && !root.buffering) {
        root.read_callback = null;
      }
    }
  };
