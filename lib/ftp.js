...
module.exports = function ftp(options) {
  var root = this;
  if (!options) {
    options = {host: undefined, port: undefined, username: undefined, password: undefined, local_ip: undefined};
  }
  // properties
  ...
  this.local_ip = options.local_ip;
  ...
  this.ondir = function() {};
  this.read_callback = []; // callback queue for 'buffering' responses.
  ...
  this._read_encrypted = function(data) {
    ...
    try { data = root._excess_buffer + data.toString(); } catch(err) {};
    root._excess_buffer = '';
    if (root.buffering) {
    ...
    if (root.read_callback.length > 0 && !root.buffering) {
      var callback = root.read_callback.shift();
      root.res_buffer = '';
      callback(ftp_data);
    } else {
      ftp_data.forEach(function(ftp_res) {
        switch (ftp_res.code) {
          case 1: // dirlist
            root.ondir(ftp_res.dirinfo);
            break;
          case 200:
            root.say('CMD '+ftp_res.msg); break;
          case 213: break;
          case 230:
            root.say('WLC '+ftp_res.msg); if (ftp_res.msg.match(/logged in/i) || ftp_res.eol) { root.onloggedin(true); }; break;
          case 257:
            root.say('PWD '+ftp_res.msg); break;
          case 331:
            root.encrypted.write('PASS '+root.password+'\n'); break;
          case 421:
            root.say('QUIT '+ftp_res.msg); break;
          case 503:
            root.encrypted.write('USER '+root.username+'\n'); break;
          case 530:
            root.say('UPW bad username/password combination');
            root.close();
          default:
            root.say('UNK '+ftp_res.code+': '+ftp_res.msg);
        }
      });
    }
  };
  ...
  this.connect = function(obj) {
    ...
    if (this.local_ip)
      this.unencrypted = new net.Socket({handle: net._createServerHandle(this.local_ip)});
    else
  ...
  this.parse_data = function(data) {
    ...
    for (var i = 0, j = lines.length; i < j; i++) {
      ...
      if (!ftp_line_m) {
        ...
        line_objs.push({code: parseInt(ftp_line_m[1]), eol: (ftp_line_m[2] === '-') ? false : true, msg: ftp_line_m[3]});
        if (ftp_line_m[1] === ' ' && (j > i + 1)) {
          root._excess_buffer = lines.slice(i + 1).join('\r\n');
        }
      }
    }
    return line_objs;
