...
module.exports = function ftp(options) {
  ...
  this._read_encrypted = function(data) {
    ...
    if (root._excess_buffer.length > 0) { data = root._excess_buffer + data.toString(); }
    root._excess_buffer = '';
    if (root.buffering) {
    ...
    if (root.read_callback[0] && !root.buffering) {
      ...
      ftp_data.forEach(function(ftp_res) {
        switch (ftp_res.code) {
          ...
          case 213: break;
          case 230:
            root.say('WLCM '+ftp_res.msg); if (ftp_res.msg.match(/logged in/i)) { root.onloggedin(true); }; break;
          case 257:
          ...
          case 530:
            ...
            root.close();
          default:
            root.say('UNKN '+ftp_res.code+': '+ftp_res.msg);
        }
      });
    }
  };
  ...
  this.cmd_queue = [];
  this.last_cmd_time = 0;

  this.cmd = function(text, callback) {
    root.cmd_queue.push({cmd: text, callback: callback});
    this._process_queue(function() {
      //console.log('cmd run');
    });
  };

  this._process_queue = function(cb) {
    while (root.cmd_queue.length > 0) {
      root._cmd(root.cmd_queue[0].cmd, root.cmd_queue[0].callback, done);
      root.cmd_queue = root.cmd_queue.slice(1);
    }
    cb();
  };
  ...
  this.cmd = function(text, callback) {
    if (root.encrypted.write) {
      ...
      root.cmd_queue_clear = false;
      root.write_encrypted(text, true, callback, done);
